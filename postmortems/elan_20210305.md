...
The issue was detected by `Asklepian`, which was guarded against the "impossible" situation of a sample missing both the `recieved_date` and `collection_date`.
...
* Need a **complete** view of how updates to Majora are made when changing the `add` and `update` interface.
...
* [ ] Suggest if someone at COG or PHA could assemble a flow chart of desired update procedures to work from in future
